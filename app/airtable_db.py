...
class AirtableDocumentDatabase:
    ...
    def __init__(
        ...
        formats_table_id: Optional[str] = None,
        featured_documents_table_id: Optional[str] = None,
        ui_strings_table_id: Optional[str] = None,
    ):
        if token is None:
        ...
        if formats_table_id is None:
            formats_table_id = os.environ["AIRTABLE_FORMATS_TABLE_ID"]
        if featured_documents_table_id is None:
            featured_documents_table_id = os.environ[
                "AIRTABLE_FEATURED_DOCUMENTS_TABLE_ID"
            ]
        if ui_strings_table_id is None:
            ui_strings_table_id = os.environ["AIRTABLE_UI_STRINGS_TABLE_ID"]
        self.api = Api(token)
        self.documents_table = self.api.table(base_id, documents_table_id)
        self.themes_table = self.api.table(base_id, themes_table_id)
        self.formats_table = self.api.table(base_id, formats_table_id)
        self.featured_documents_table = self.api.table(
            base_id, featured_documents_table_id
        )
        self.ui_strings_table = self.api.table(base_id, ui_strings_table_id)
        self.documents: dict[str, Document] = {}
        self.themes: OrderedDict[str, Theme] = OrderedDict()
        self.tags: dict[str, list[str]] = defaultdict(list)
        for row in self.themes_table.all():
            fields = row["fields"]
            id = row["id"]
            self.themes[id] = {
                "name": fields["name"],
        ...
        for row in self.documents_table.all():
            ...
            if format_id is not None:
                format = self.formats_table.get(format_id)["fields"]
